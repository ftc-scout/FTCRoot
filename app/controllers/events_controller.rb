  ...
  def index
    ...
    @events = @events.where("advanceddata is not null")
      if(ActiveRecord::Base.connection.adapter_name == 'Mysql2' )
          @events = @events.order(Arel.sql( 'STR_TO_DATE(date, "%m/%d/%Y") DESC, name ASC' )).paginate(:page => params[:page], :per_page => 20)
      else
          @events = @events.order(Arel.sql( 'to_date(date,\'MM/DD/YYYY\') DESC, name ASC' )).paginate(:page => params[:page], :per_page => 20)
      end
  end
